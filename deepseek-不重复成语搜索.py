...
#成语清单
idiom_list = [] #idiom_list = ["四面楚歌", "五湖四海", "三心二意", "七上八下", "八面玲珑"]

#构建成语清单，过滤掉包含非常用汉字的成语、本身存在重复字的成语、长度超过4个字的成语
for data in idiom_info_list:
    idiom = data['word']
    if(len(set(list(idiom))) == 4):

        #过滤掉非常用汉字
        common = 0
        for ch in list(idiom):
            if ch not in common_char_list:
                common = 1
                break
        if common == 1:
            continue
...
'''
请编写一个python程序，该程序可以从给定的成语清单里面按照指定规则挑选出符合规则的成语，且数量最大化。【规则】成语本身及成语之间不允许出现重复汉字，且成语最大长度为4个汉字

可以增加一个截断么，当成语数量超过400就可以停止搜索了
'''
def find_max_compounded_chengyu(chengyu_list, MAX_LEN):
    # 过滤出符合条件的成语：长度不超过4且无重复汉字
    candidates = []
    for cy in chengyu_list:
        if len(cy) <= 4 and len(set(cy)) == len(cy):
    ...
    # 根据汉字出现次数和成语长度对候选列表进行排序
    sorted_candidates = sorted(candidates, key=lambda x: (sum(char_count[c] for c in x), len(x)))
    
    max_combination = []
    max_length = 0
    found = False  # 新增截断标志
    
    def backtrack(start, used_chars, path):
        nonlocal max_length, max_combination, found
        if found:
            return  # 发现截断标志后立即终止
        ...
        if len(path) > max_length:
            max_length = len(path)
            max_combination = list(path)
            if max_length >= MAX_LEN:  # 达到截断条件
                found = True
                return
            else:
                print(max_length)
        
        # 剪枝：剩余候选不足以超越当前最大值则提前退出
        remaining = len(sorted_candidates) - start
        ...
        for i in range(start, len(sorted_candidates)):
            if found:
                break  # 截断后续循环
            cy = sorted_candidates[i]
            cy_chars = set(cy)
            if cy_chars.isdisjoint(used_chars):
                # 选择当前成语
                path.append(cy)
                backtrack(i + 1, used_chars.union(cy_chars), path)
                path.pop()
...
# 示例成语清单
idiom_list = [
    "一马当先", "二话不说", "三顾茅庐", "四面楚歌", "五湖四海",
    "六神无主", "七窍生烟", "八面玲珑", "九牛一毛", "十全十美"
]

# 找到符合规则的最大成语组合
result = find_max_compounded_chengyu(idiom_list,500)
